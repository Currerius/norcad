...
# `r sapply( JSON$Endpoints , with , Label )[n.Surv]` and `r sapply( JSON$Predictors , with , Label )[n.P]` in strata of `r sapply( JSON$Modifiers , with , Label )[n.M]`
...
The Endpoint `r sapply( JSON$Endpoints , with , Label )[n.Surv]` is observed in `r sum(D$E0 , na.rm = TRUE )` of `r dim(D)[n.Surv]` patients (`r round(100*sum(D$E0 , na.rm = TRUE )/dim(D)[n.Surv] , 1)`%) with a median follow-up time of `r median(D[ , paste("T",n.Surv-1,sep="")],na.rm=TRUE)` days.
...
### HR per SD-increase of `r sapply( JSON$Predictors , with , Label )[n.P]`
      
```{r,TabzCoxInteraction,results='asis'}
...
### HR per SD increase of log-transformed `r sapply( JSON$Predictors , with , Label )[n.P]`

```{r,TabzlogCoxInteraction,results='asis'}
...
### HR per quartiles of `r sapply( JSON$Predictors , with , Label )[n.P]`

```{r,Tabn4CoxInteraction,results='asis'}
...
```{r,CoxGAMqSubgroups,fig.width=16,fig.height=8,out.width="800pt"}
...
```{r,CoxGAMzlogSubgroups,fig.width=16,fig.height=8,out.width="800pt"}
...
```{r,CoxSmoothSubgroupsq,fig.width=8,fig.height=8,out.width="600pt"}
...
```{r,CoxSmoothSubgroupszlog,fig.width=8,fig.height=8,out.width="600pt"}
...
```{r,VisGAMq,fig.width=8,fig.height=8,out.width="600pt"}
...
```{r,VisGAMqSE,fig.width=8,fig.height=8,out.width="600pt"}
