...
D$Pred <- D[ , Predictor ]
D$zPred <- D[ , Predictor ]
D$zlogPred <- D[ , Predictor ]
D$qPred <- D[ , Predictor ]
D$Pred2n <- as.numeric(cutN( D[ , Predictor ] , n = 2 ))
D$Pred2c <- cutN( D[ , Predictor ] , n = 2 )
D$Pred3n <- as.numeric(cutN( D[ , Predictor ] , n = 3 ))
D$Pred3c <- cutN( D[ , Predictor ] , n = 3 )
D$Pred4n <- as.numeric(cutN( D[ , Predictor ] , n = 4 ))
D$Pred4c <- cutN( D[ , Predictor ] , n = 4 )
D$Pred5n <- as.numeric(cutN( D[ , Predictor ] , n = 5 ))
D$Pred5c <- cutN( D[ , Predictor ] , n = 5 )
...
T <- lapply(
    X = PatientCharacteristics ,
    FUN = MeansOverGroups ,
    groups = Pred4 )
    

Table1Proportions <- rbind(
    ProportionsOverGroups("male","tHcy4") ,
    ProportionsOverGroups("current_smoking","tHcy4") ,
    ProportionsOverGroups("obesity","tHcy4")
)


attr( Table1Proportions , "dimnames")[[1]] <- c(
    "Male" ,
    "Current smoker" ,
    "Obesity (BMI>30)")
...
write.csv2(
    ...
    file = gsub(
        ...
        "_" ,
        paste(
            "./export/Table1_Proportions_over_quartiles_of_" ,
            JSON$Predictor$ShortLabel ,
            ".csv" ,
            sep = ""
        )
    )
...
@ 
Smoothing spline estimates of \Sexpr{JSON$Endpoint$Label} hazard by \Sexpr{JSON$Predictor$Label} in subgroups of high vs. low levels of \Sexpr{JSON$Modifier$Label} estimated with Cox proportional hazards models adjusted for \Sexpr{paste0( sapply( JSON$Adjustments , with , Label )[sapply( JSON$Adjustments , with , Level )==1] , collapse = ', ')}. The effect modifier \Sexpr{JSON$Modifier$Label} is divided into high/low levels according to the population median of \Sexpr{signif( median( D[ , Modifier ] , na.rm = TRUE ),3)}.
\end{frame}
